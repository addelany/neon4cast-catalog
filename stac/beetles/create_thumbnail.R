...
for (m_id in theme_models$model.id[1:2]){
  ...
  for (site in unique(info_df$site_id)){
    ...
    baseline_site_df <- baseline_df |>
      ...
      rename(base_crps = crps) |>
      select(datetime,variable, base_crps)

    if (nrow(baseline_site_df) == 0){
      print(paste0('no baseline forecast for ',{site}, ' on ', {latest_scores_site$reference_datetime}))
      next
    }
    ...
    scores_plot <- ggplot(data = latest_scores_df, aes(datetime, crps)) +
      geom_line(color = "steelblue", linewidth = 1) +
      geom_line(aes(y = base_crps), color = 'darkred', linetype = 'dashed') +
